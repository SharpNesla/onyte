...
export interface CreateFileDialogOptions {
...
const defaultOptions: CreateFileDialogOptions = {
  multiple: true,
  accept: '*',
};

function getInitialFilesList(files: CreateFileDialogOptions['initialFiles']): FileList | null {
  if (!files) {
    return null;
  }
...
function createInput(options: CreateFileDialogOptions): HTMLInputElement | null {
...
export interface CreateFileDialogReturnValue {
...
export function createFileDialog(
  input: CreateFileDialogOptions = {}
): CreateFileDialogReturnValue {
  const options: CreateFileDialogOptions = { ...defaultOptions, ...input };
